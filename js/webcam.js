...
window.webcam = {
	...
	swf_url: '/wp-content/themes/responsive-new/js/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					...
					// Remove webcam access button
					var webcamButton = document.getElementById('webcamButton');
					var webcamLanguage = document.getElementById('webcamLanguage');
					webcamButton.style.visibility = "hidden";
					webcamButton.style.width = "0px";
					while( webcamLanguage.firstChild ) {
						webcamLanguage.removeChild( webcamLanguage.firstChild );
					}
